...
import { subscript, superscript } from "./subscript_superscript.json" with {
	type: "json",
};
...
export default function LaTeXToUnicode() {
	...
	function replace(str) {
		...
		val = val
			.replaceAll(/_\{(.*?)\}/g, (_, sub) =>
				sub.replaceAll(/./g, (sub) => subscript[sub] ?? sub),
			)
			.replaceAll(/\^\{(.*?)\}/g, (_, sub) =>
				sub.replaceAll(/./g, (sup) => superscript[sup] ?? sup),
			)
