...
<template>
  <div class="pdf-viewer-container bg-secondary bg-opacity-10 p-3 rounded overflow-auto" style="max-height: 800px;">
    ...
    <div v-else-if="pages.length > 0" class="d-flex flex-column gap-3 align-items-center">
      <div v-for="(page, index) in pages" :key="index" class="pdf-page shadow-sm bg-white p-1 position-relative">
        <img :src="page" class="img-fluid border" :alt="'Page ' + (index + 1)" />
        <span class="badge bg-dark position-absolute top-0 start-0 m-2 opacity-50">Page {{ index + 1 }}</span>
      </div>
    </div>
